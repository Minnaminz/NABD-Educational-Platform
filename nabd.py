...
"""
    ...
    for attempt in range(max_attempts):
        try:
            ...
            if len(data) != count:
                raise ValueError(
                    f"Gemini returned {len(data)} questions instead of {count}."
                )
            ...
            for i, item in enumerate(data, start=1):
                ...
                if not required_keys.issubset(item):
                    raise ValueError(
                        f"Question {i} is missing required fields."
                    )
                ...
                answer_as_text = str(item["answer"]).strip()

                if answer_as_text not in options_as_text:
                    raise ValueError(
                        f"Question {i} has an answer that is not one of its options."
                    )
...
QUESTIONS = [
    {
        ...
        "question_en": "Solve for x: x + 5 = 12",
        "options": ["5", "6", "7", "8"],
        "answer": "7",
        "solution_ar": "س = ١٢ - ٥ = ٧",
        "solution_en": "x = 12 - 5 = 7"
    },
    {
        "id": 2,
        "concept": "Algebra",
        ...
        "question_en": "Solve for x: 2x = 10",
        "options": ["2", "5", "8", "10"],
        "answer": "5",
        "solution_ar": "س = ١٠ ÷ ٢ = ٥",
        "solution_en": "x = 10 / 2 = 5"
    },
    {
        "id": 3,
        "concept": "Arithmetic",
        ...
        "options": ["2", "5", "10", "15"],
        "answer": "5",
        "solution_ar": "٠٫١ × ٥٠ = ٥",
        "solution_en": "0.1 * 50 = 5"
    },
    {
        "id": 4,
        "concept": "Arithmetic",
        "skill": "Multiplication",
        "question_ar": "ما حاصل ضرب ١٢ × ٨؟",
        "question_en": "What is 12 x 8?",
...
with st.sidebar:
    st.markdown(
        "<h1 style='text-align: center;'>🧠 NABD</h1>",
        unsafe_allow_html=True
    )
...
if st.session_state.page == "Home":
    ...
    c1.metric(
        f"🔥 {L['streak']}",
        f"{st.session_state.streak} {L['days']}"
    )

    c2.metric(
        f"⭐ {L['points']}",
        f"{st.session_state.user_points}"
    )
...
# =========================================================
    ...
    if not st.session_state.assessment_submitted:
        ...
        st.subheader(
            f"Q{idx + 1}/{len(q_list)}: "
            f"{q['question_en' if st.session_state.lang == 'English' else 'question_ar']}"
        )
        ...
        if idx < len(q_list) - 1:
            ...
            if col_n.button(L["finish"]):
                correct = sum(
                    1
                    for item in q_list
                    if st.session_state.assessment_answers.get(item["id"])
                    == item["answer"]
                )

                score = round(
                    (correct / len(q_list)) * 100
                )
        ...
        play_audio_and_effects(
            score,
            "assessment_celebration_shown"
        )

        st.success(f"Score: {score}%")

        st.markdown(
            f"### {L['score_msg_high'] if score >= 75 else L['score_msg_low']}"
        )
...
# =========================================================
    ...
    if st.session_state.before_score is not None:
        st.metric(
            "Assessment Score",
            f"{st.session_state.before_score}%"
        )

        st.progress(
            st.session_state.before_score / 100
        )
        ...
        for item in q_list:
            u_ans = st.session_state.assessment_answers.get(
                item["id"]
            )
            ...
            with st.expander(
                f"{status} Skill: {item['skill']} ({item['concept']})"
            ):
                st.write(
                    f"**Question:** "
                    f"{item['question_en' if st.session_state.lang == 'English' else 'question_ar']}"
                )
                ...
                st.write(
                    f"💡 **Solution:** "
                    f"{item['solution_en' if st.session_state.lang == 'English' else 'solution_ar']}"
                )
...
# =========================================================
    ...
    st.write(
        "AI-powered Decision Tree model analyzing common misconceptions."
    )

    if clf is not None:
        sample_concept = st.selectbox(
            "Select Concept:",
            df["concept"].unique()
        )

        sample_skill = st.selectbox(
            "Select Skill:",
            df[
                df["concept"] == sample_concept
            ]["skill"].unique()
        )

        sample_correct = st.radio(
            "Attempt Status:",
        ...
        if st.button(
            ...
            pred_error = clf.predict(
                input_encoded
            )[0]

            st.info(
                f"🎯 **Diagnostic Result:** "
                f"Detected pattern -> **{pred_error}**"
            )
...
# =========================================================
    ...
    st.markdown(
        f"### {p_q['question_en' if st.session_state.lang == 'English' else 'question_ar']}"
    )
    ...
    if st.button(L["submit"]):
        if user_p == p_q["answer"]:
            ...
            st.error(
                f"Incorrect. The correct answer is {p_q['answer']}"
            )
...
# =========================================================
    ...
    with tab2:
        ...
        if st.button(
            ...
            if choice == "Step 1":
                ...
                st.success(
                    "🎉 Correct! Moving +5 to the other side "
                    "requires subtraction (-5), not addition."
                )

            else:
                st.error(
                    "❌ Try again! Check the sign in Step 1."
                )
    ...
    with tab3:
        ...
        if st.button(
            ...
            if ans_day == "2^9":
                st.success(
                    "Correct! Daily streak maintained 🔥 (+1 Day)"
                )
            else:
                st.error(
                    "Try again tomorrow!"
...
# =========================================================
    ...
    if client:
        ...
        st.caption(
            "Configuration alone does not prove the API call works. "
            "Use the test below before your competition demo."
        )
        ...
        if st.session_state.ai_verified:
            st.info(
                "✅ AI connection verified in this session."
            )
        ...
        if st.button(
            ...
            with st.spinner(
                ...
                try:
                    ...
                    for i, q_item in enumerate(
                        generated
                    ):
                        st.markdown(
                            f"**Q{i+1}: {q_item['question']}**"
                        )

                        st.write(
                            f"Options: "
                            f"{', '.join(q_item['options'])}"
                        )
...
# =========================================================
    ...
    if not st.session_state.reassess_submitted:
        ...
        if not q_list:
            st.session_state.reassess_questions = random.sample(
                QUESTIONS,
                len(QUESTIONS)
            )
        ...
        if idx < len(q_list) - 1:
            ...
            if st.button(L["finish"]):
                correct = sum(
                    1
                    for item in q_list
                    if st.session_state.reassess_answers.get(
                        item["id"]
                    ) == item["answer"]
                )

                st.session_state.after_score = round(
                    (correct / len(q_list)) * 100
                )
        ...
        play_audio_and_effects(
            after,
            "reassessment_celebration_shown"
        )

        st.success(
            f"Reassessment Score: {after}%"
        )
